"""By-state district-clustered TWFE regression with NDBI_t (level) as the outcome (linearmodels cross-check).

Replicates run_ndbi_ols_by_state.py (pyfixest primary) using linearmodels PanelOLS,
run separately on each of the 4 states. Same specification as run_ndbi_pooled.py.
...
def run_state(df_state, suffix):
    ...
    X = pd.DataFrame({
        ...
        f"NDBI_{suffix}_t_minus_1": df_state[f"NDBI_{suffix}_t_minus_1"],
    }, index=df_state.index)
    X["const"] = 1.0

    panel = pd.concat([y.rename("ndbi_level"), X, df_state[["AC_UID", "YEAR", "DISTRICT_ID"]]], axis=1).dropna()
...
def main():
    ...
    notes = pd.DataFrame({
        "Note": [
            ...
            "Cross-check: same specification as run_ndbi_ols_by_state.py (pyfixest); coefficients should be close.",
        ]
    })
