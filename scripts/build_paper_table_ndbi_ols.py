...
def get_pooled(model):
    ...
    out["ndbi_lag"] = (r["Coefficient"], r["Std_Error"], r["Significance"])
...
def get_state(model, state):
    ...
    return {
        ...
        "nl": row(f"NL_{suffix}_t_minus_1"),
        "ndbi_lag": row(f"NDBI_{suffix}_t_minus_1"),
...
def build_sheet(ws, model_name):
    ...
    ws.cell(row=1, column=1, value="Dependent variable: NDBI_t — TWFE-LDV (pyfixest)")
...
notes = [
    ...
    "",
    "Estimator: TWFE-LDV via pyfixest. AC FE = Yes, Year FE = Yes. Lagged DV (NDBI_{t-1}) on RHS.",
    "SEs clustered by district (correlated shocks within districts).",
    "",
    "============================================================",
    "",
    "Source files:",
    "  - Regression_Results_NDBI_OLS_Pooled.xlsx       (All states column)",
    "  - Regression_Results_NDBI_OLS_By_State.xlsx     (Bihar / Jharkhand / Odisha / WB columns)",
    "",
    "Specification:",
    "  NDBI_it = β₁·Seasonal_Ratio_it + β₂·NDVI_{i,t-1} + β₃·NL_{i,t-1} + β₄·NDBI_{i,t-1}",
    "           + α_i + γ_t + ε_it",
    ...
    "",
    "Nickell-bias caveat:",
    "  Including NDBI_{t-1} on the RHS together with AC fixed effects in a short panel (T=5)",
    "  introduces a downward bias on β₄ of order 1/T ≈ 20%. The flood coefficient (β₁) is far",
    "  less affected. Bias on β₂ and β₃ is also second-order. Read β₄ as a lower bound on",
    "  NDBI persistence; do not over-interpret its magnitude.",
    "",
    "Variable mapping:",
